...
Entry = import_model(getattr(settings, 'BILA_ENTRY_MODEL', EntryAbstract))
...
def _add_owner_panel():
    ...
    `settings_panels` if Bila is using a version greater or equal than 1.11.
...
class EntryPage(six.with_metaclass(PageBase, Entry, Page)):
    ...
    parent_page_types = ['bila.BlogPage']
    subpage_types = []
